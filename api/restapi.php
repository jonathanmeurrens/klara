...
function getInfoForArtist(){
    ...
    $artist = Slim::getInstance()->request()->params("artist");
    $artist = myUrlEncode(utf8_encode(str_replace(" ","+",$artist)));
   // echo "{artist:'".$artist."'}";
    //exit();
    $xml_string = file_get_contents('http://musicbrainz.org/ws/2/artist/?query='.$artist);
    //$xml_string = file_get_contents('http://musicbrainz.org/ws/2/artist/?query=Abbuehl+Susanne');
    //$xml_string = file_get_contents('http://musicbrainz.org/search?query='.urlencode(Slim::getInstance()->request()->params("artist")).'&type=artist&method=indexed');
